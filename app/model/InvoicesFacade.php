...
class InvoicesFacade extends Facade
{
	 ...
	 * @return array
	 */
	public function findAll($query, $state, $offset = 0, $limit = 20, $userId)
	{
		...
		if ($query) {
			...
			$selection->where('(
				...
				CAST(payed AS text) LIKE ? OR
				CAST(create_date AS text) LIKE ? OR
				CAST(end_date AS text) LIKE ?
